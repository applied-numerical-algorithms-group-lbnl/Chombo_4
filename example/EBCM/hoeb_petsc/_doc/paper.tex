...
\begin{equation}
  D_\fbold{V_1) = \sum\limits_{d \in {1...D}} |\vbold^d - \ibold_0^d|.
\label{eqn::uptownFunk}
\end{equation}
$\gbold_\fbold(V_1)$, the weight of the $V_1$ equation in $\fbold$ system
is given by
\begin{equation}
  \gbold_\fbold(V_1))  \equiv \frac{1}{D\fbold(\V1)^{P_w})},
...
$$
One would like to avoid inverting $A$ every time one withes to apply
...
$$
Since this has to work for all $\psi$, we can define the stencil as
$$
...
signalling poorly-conditioned matrices.    Their data shows the
