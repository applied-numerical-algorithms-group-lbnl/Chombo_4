...
and not computationally overwhelming \cite{trebotich-graves}.
...
accuracy \cite{geometry_paper}.

EB grids can produce cells that arbitrarily small.    This produces
...
compressible flows \cite{compresible_ns_paper, ebamrhscl} and for
slower, incompressible applications \cite{miller-trebotich}.
...
products of grid generation.
Modern, higher order embedded boundary methods use this description
directly to generate stencils
 \cite{nate-stokes-paper,  dharshi-poisson, geometry-paper}.    At the core
of these algorithms is a similar procedure for finding the Taylor
coefficients $C$ which is
worth discussing.

\section{Computing Taylor coefficients}
...
\end{equation}
Let's say there are $N_p$ Taylor coefficients.   We can force
$N_v > N_p$ by getting a large enough neighborhood.    This means that
we have an overdetermined system of the form
\begin{equation*}
M C = P 
\end{equation*}
where $M = \{\{m^{pi}_\jbold\}\}$ and $P= \{<\psi_\jbold>\}$.
Since the system is overdetermined, we can introduce a 
meaningful weighting matrix $W$ and solve
\begin{equation*}
WMC = WP.
\end{equation*}
Diagonal weighing matricies are common.  Typically, the equations for volumes near the
target volume receive higher weights.   Since these matricies are
(deliberately) not square, we approximate the solution using a
Moore-Penrose pseudo-inverse:
\begin{equation*}
C = ((WM)^T(WM))^{-1} (WM)^T P.
\end{equation*}
The matrix inversion at the heart of this  method requires that we
need know to what degree the matrix $A = (WM)^T(WM)$  is invertable.
