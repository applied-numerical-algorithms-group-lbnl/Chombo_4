...
\footnotetext[6] {The discerning reader will notice that this
  discussion assumes that the equation set used to create $C$ only
  includes the volume equations.   Devendran \cite{Devendran2017} also
  include boundary condition equations in the equation set.  For
  simple enough boundary conditions, this works very well.   There are
  many equation sets, however, which do not lend themselves easily to
  this approach.   Notably, hyperbolic equations often have boundary 
  conditions which are fomulated in terms of characteristic
  variables.    Using this information in the system of equations used
  to calculate Taylor coefficients is still a matter of research.}
